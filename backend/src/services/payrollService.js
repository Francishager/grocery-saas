...
import { calculateUgandaSalaryPreview, normalizeSalaryInput } from "../utils/ugandaSalaryPreview.js";
...
class PayrollService {
  ...
  async calculateSalaryPreview(params) {
    try {
      normalizeSalaryInput(params);
      let employee = null;
      if (params.employeeId) {
        employee = await prisma.employee.findFirst({
          where: { id: params.employeeId, tenantId: params.tenantId },
          select: {
            id: true, employeeNumber: true, firstName: true, middleName: true,
            lastName: true, taxId: true, socialSecurityNumber: true, basicSalary: true, status: true,
          },
        });
        if (!employee) return { success: false, error: "Employee not found" };
      }
      return calculateUgandaSalaryPreview(params, employee);
    } catch (error) {
