...
const initial = { kind: 'flyer', brief: '', reportType: 'sales', tone: 'friendly', platform: 'facebook', format: 'portrait', palette: 'green', artwork: 'auto', productId: '' }
...
export default function AdvisorCreativeStudio({ conversationId, disabled, days, ensureConversation }: {
  ...
  async function save() {
    ...
    setBusy(true); setError('')
    try {
      const { headline, subheading, body, cta, caption } = copy
      const data = await request(`/artifacts/${encodeURIComponent(selected.id)}`, { method: 'PATCH', body: JSON.stringify({ headline, subheading, body, cta, caption }) })
          ...
          <fieldset disabled={busy} className="space-y-4">
            ...
            <div className="grid min-w-0 gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {form.kind === 'report' ? <label className="block min-w-0 text-sm">Report<select aria-label="Report type" required value={form.reportType} onChange={event => setForm({ ...form, reportType: event.target.value })} className={field}><option value="" disabled>Choose report</option>{reports.map(type => <option key={type} value={type}>{reportNames[type]}</option>)}</select></label> : <>
                ...
                <label className="block min-w-0 text-sm">Artwork<select aria-label="Artwork" value={form.artwork} onChange={event => setForm({ ...form, artwork: event.target.value })} className={field}><option value="auto">Automatic</option><option value="product">Product photo</option><option value="generated">AI illustration</option><option value="none">Text only</option></select></label>
              </>}
          ...
          <div className="grid min-w-0 gap-5 lg:grid-cols-[minmax(0,1.2fr)_minmax(260px,0.8fr)]">
            ...
            <div className="min-w-0 space-y-4">
              ...
              {editing && copy ? <form onSubmit={event => { event.preventDefault(); void save() }} className="space-y-3">
                {(selected.kind === 'report' ? [['body', 'Report commentary', 320], ['caption', 'Summary', 2400]] : [['headline', 'Headline', 100], ['subheading', 'Subheading', 180], ['body', 'Body', 320], ['cta', 'Call to action', 70], ['caption', 'Caption', 2400]]).map(([key, label, max]) => <label key={key} className="block text-sm">{label}<textarea aria-label={String(label)} required={key === 'headline'} maxLength={Number(max)} rows={key === 'caption' ? 5 : 2} value={copy[key as keyof Omit<CreativeCopy, 'hashtags'>]} onChange={event => setCopy({ ...copy, [key]: event.target.value })} className={field} /></label>)}
