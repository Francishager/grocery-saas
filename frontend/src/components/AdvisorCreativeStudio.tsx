          ...
          <fieldset disabled={busy} className="space-y-4">
            ...
            <div className="grid min-w-0 gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {form.kind === 'report' ? <label className="block min-w-0 text-sm">Report<select aria-label="Report type" required value={form.reportType} onChange={event => setForm({ ...form, reportType: event.target.value })} className={field}><option value="" disabled>Choose report</option>{reports.map(type => <option key={type} value={type}>{reportNames[type]}</option>)}</select></label> : <>
                ...
                <label className="block min-w-0 text-sm">Artwork<select aria-label="Artwork" value={form.artwork} onChange={event => setForm({ ...form, artwork: event.target.value })} className={field}><option value="auto">Automatic</option><option value="product">Business product photo</option><option value="external">External photo</option><option value="none">Text only</option></select></label>
              </>}
        ...
        {view === 'preview' && selected?.data && <div className="space-y-3">
          ...
          <div className="grid min-w-0 gap-5 lg:grid-cols-[minmax(0,1.2fr)_minmax(260px,0.8fr)]">
            ...
            <div className="min-w-0 space-y-4">
              ...
              {selected.data.externalPhoto && <p className="break-words text-xs text-muted-foreground"><a href={selected.data.externalPhoto.sourceUrl} target="_blank" rel="noopener noreferrer" className="underline">Photo source</a> | {selected.data.externalPhoto.license}{selected.data.externalPhoto.creator ? ` | ${selected.data.externalPhoto.creator}` : ''}</p>}
              ...
              <Button variant="ghost" size="sm" disabled={busy || editing} onClick={() => { retryRef.current = null; setForm({ ...initial, kind: selected.kind, brief: selected.data!.brief, layout: selected.data!.layout || 'auto', format: selected.data!.format, palette: selected.data!.palette, platform: selected.data!.platform, tone: selected.data!.tone || initial.tone, productId: selected.data!.productId || '', artwork: selected.data!.artwork || 'auto', reportType: selected.data!.reportType || initial.reportType }); create() }}><Plus className="mr-1 h-4 w-4" />Another version</Button>
            </div>
          </div>
