...
test('advisor saves, organizes, reopens and safely renders chats on desktop, tablet and phone', { timeout: 240000 }, async () => {
  const adapters = {
  ...
  try {
    for (const viewport of [{ width: 1440, height: 1000 }, { width: 1024, height: 768 }, { width: 768, height: 1024 }, { width: 390, height: 844 }, { width: 320, height: 740 }]) {
      ...
      const errors = [], collections = [], chats = [], turns = [], requests = [];
      let failOnce = true;
      ...
      const mount = async url => { await page.goto(url || 'http://localhost:4179/tenant/ai-advisor'); await page.addStyleTag({ content: css }); await page.addScriptTag({ content: bundle.outputFiles[0].text }); await page.getByRole('heading', { name: 'AI Advisor', exact: true }).waitFor(); };
